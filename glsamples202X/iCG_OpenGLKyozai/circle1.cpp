...
void makeWeb( void )
{
	...
	for( int i = 0; i < NR; i++ ){
		...
		for( int j = 0; j < NA; j++ ){
			...
			switch( projmode ){
			case 0:
				l = equidistance( r ); //等距離射影
				break;
			case 1:
				l = equisolidangle( r ); //等立体角射影
				break;
			case 2:
				l = stereographic( r ); //立体射影
				break;
			case 3:
				l = orthogonal(  r ); //正射影
				break;
...
void drawWeb( void )
{
	...
	glBegin( GL_LINES );
	for( int j = 0; j < NA; j++ ){
		//glVertex2f( webx[0][j], weby[0][j] );
		glVertex2f( 0.0, 0.0 );
		glVertex2f( webx[NR-1][j], weby[NR-1][j] );
	}
	glEnd();
