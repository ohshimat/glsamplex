...
public:
	...
	STDMETHODIMP BufferCB(double dblSampleTime, BYTE *pBuffer, long lBufferSize)
	{
		...
		ewc_s[i].ftime= dblSampleTime -ewc_s[i].buf_stime[prev_index(ewc_s[i].buffer_w)];
		ewc_s[i].buf_stime[ewc_s[i].buffer_w]= dblSampleTime;
		ewc_s[i].w_count++;
		ewc_s[i].buf_count[ewc_s[i].buffer_w]= ewc_s[i].w_count;
		ewc_mrls(i);
		return S_OK;
	}
	//コンストラクタ	
...
int EWC_IsCaptured(int num, int *buffer_r= NULL)
{
	if(numCheck(num)) return 0;
	ewc_mget(num);
	int dif= ewc_s[num].buf_count[ewc_s[num].buffer_r] -ewc_s[num].r_count;
...
int EWC_GetImage(int num, void *buffer, int *count= NULL, double *stime= NULL)
{
	...
	ewc_mget(num);
	unsigned char *p= (unsigned char*)ewc_s[num].pbuf + ewc_s[num].bufsize * ewc_s[num].buffer_r;
	memcpy(buffer, p, ewc_s[num].bufsize);
	ewc_s[num].r_count= ewc_s[num].buf_count[ewc_s[num].buffer_r];
	if(count) *count= ewc_s[num].r_count;
	if(stime) *stime= ewc_s[num].buf_stime[ewc_s[num].buffer_r];
	if(ewc_s[num].buffer_r!=ewc_s[num].buffer_w) ewc_s[num].buffer_r= next_index(ewc_s[num].buffer_r);
	ewc_mrls(num);
	return 0;
}
