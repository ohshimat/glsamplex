...
//ウィンドウ位置・サイズ
const int WINDOW_XO = 0;
const int WINDOW_YO = 0;
//const float WINDOW_K = 0.375;//実サイズに合わせるための調整
const float WINDOW_K = 0.8;
...
//シーンデータ
float rot = -90.0;//オブジェクトを回転させる角度パラメータ
...
void display(void)
{
	...
	//◆--------右眼画像描画
	//glViewport(VIEWPORT_W, 0, VIEWPORT_W, VIEWPORT_H);//◆右眼画像領域指定
	glPushMatrix();
	ProjWall( head_x, head_y, head_z );//右視点透視投影変換＆
	//glTranslatef(-IPD / 2.0, 0.0, 0.0);//視差分の相対ビューイング変換
	Draw();//レンダリング
	glPopMatrix();
	/*
	//◆--------左眼画像描画
	glViewport(0, 0, VIEWPORT_W, VIEWPORT_H);//◆左眼画像領域指定
	glPushMatrix();
	ProjWall(head_x, head_y, head_z);//左視点透視投影変換
	glTranslatef( IPD / 2.0, 0.0, 0.0);//視差分の相対ビューイング変換
	Draw();//レンダリング
	glPopMatrix();
	*/
